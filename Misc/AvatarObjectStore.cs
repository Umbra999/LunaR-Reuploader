...
namespace LunarUploader.Misc 
{
    internal class AvatarObjectStore : FileObjectStore
    {
        ...
        internal AvatarObjectStore(VRChatApiClient client, string name, string unityversion, string path, bool quest = false, CancellationToken? ct = null) : base(client, path) 
        {
        ...
        internal override async Task Reupload() 
        {
            try 
            {
                ...
                if (!await CustomApiFileHelper.UploadFile(_apiClient, _path, friendlyAssetBundleName, string.Empty, true, OnAvatarUploadSuccess, OnAvatarUploadFailure).ConfigureAwait(false)) Console.WriteLine("Failed to upload avatar!");
            } catch (Exception e) {
                Console.WriteLine(e);
            }
        }
        ...
        {
            return "Avatar - " + _Name + " - Asset bundle - " + unityVersion + "_4_" + platform + "_Release";
        }
    }
}
